<template>
    ...
    <div v-else-if="!isLoading && product" class="grid md:grid-cols-2 gap-8 items-start">
			<div 
				v-motion
						...
						<p
							:class="
								product.isOnSale
									...
									: 'font-bold text-indigo-600 text-2xl'
							"
						>
							${{ product.price.toFixed(2) }}
						</p>
						<p v-if="product.isOnSale" class="font-bold text-2xl text-rose-500 ml-4">
...
<script setup lang="ts">
	...
	const { product, isLoading, error, fetchItemById } = useFetchItemById<Product>();
	...
	const handleProductLoad = async () => {
		...
		const productIsInStore = products.value?.find(product => product.id === productId);
